            ...
            <div class="box">
                ...
                <div class="box-body">
                    ...
                    <div class="step active">
                        <div class="box-body">
                            <div class="row">
                                {{ Aire::number('kilometrage', 'Kilométrage (km)*')->required()->value(old('kilometrage', ( $inspection->kilometrage ?? (($type->id == \Ipsum\Reservation\app\Models\Inspection\Type::FINAL_ID && $reservation->inspection_initiale) ? $reservation->inspection_initiale->kilometrage : ($reservation->vehicule->last_inspection ? $reservation->vehicule->last_inspection->kilometrage : '')))))->helpText(( $type->id == \Ipsum\Reservation\app\Models\Inspection\Type::FINAL_ID && $reservation->inspection_initiale) ? 'Kilométrage initial : '.$reservation->inspection_initiale?->kilometrage.' km' : '')->groupAddClass('col-md-12') }}
                                {{ Aire::range('carburant', 'Niveau de carburant*')->step('1')->min(0)->max(8)->value(old('carburant', ( $inspection->carburant ?? (($type->id == \Ipsum\Reservation\app\Models\Inspection\Type::FINAL_ID && $reservation->inspection_initiale) ? $reservation->inspection_initiale->carburant : ($reservation->vehicule->last_inspection ? $reservation->vehicule->last_inspection->carburant : 8)))))->list('markers')->id('carburant')->helpText(( $type->id == \Ipsum\Reservation\app\Models\Inspection\Type::FINAL_ID && $reservation->inspection_initiale) ? 'Niveau de carburant initial : '.$reservation->inspection_initiale?->carburant.'/8' : '')->groupAddClass('col-md-11') }}
